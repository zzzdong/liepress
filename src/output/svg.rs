...
use super::common::{
    ...
    list_item_indent, table_border_segments, table_row_height,
};
...
impl SvgRenderer {
    ...
    ///
    /// - 任务列表：`<rect>` 复选框外框，勾选时加 `<polyline>` 对勾。
    /// - 无序列表（marker="●"）：矢量 `<circle>` 实心圆点。
    /// - 有序列表（marker="N."）：`<text>` 绘制数字（SVG 依赖系统字体渲染文本）。
    ///
    ...
    fn draw_list_marker(
        ...
        let fill = Self::color(&style.color);
        ...
        if is_task {
            ...
            let _ = writeln!(
                self.out,
                r#"<rect x="{:.2}" y="{:.2}" width="{:.2}" height="{:.2}" fill="none" stroke="{}" stroke-width="1.2" />"#,
                x0, y0, size, size, fill
            );
            if checked {
                let s = size;
                let _ = writeln!(
                    self.out,
                    r#"<polyline points="{:.2},{:.2} {:.2},{:.2} {:.2},{:.2}" fill="none" stroke="{}" stroke-width="1.6" />"#,
                    x0 + s * 0.2,
                    y0 + s * 0.52,
                    x0 + s * 0.42,
                    y0 + s * 0.72,
    ...
    fn draw_block(&mut self, block: &Block, x: f64, y: f64) {
        ...
        match &block.kind {
            ...
            BlockKind::ListItem { marker, children } => {
                // marker 在缩进槽左缘单独矢量绘制（圆点 / 有序数字），正文整体缩进。
                let indent = list_item_indent(marker, style);
                self.draw_list_marker(marker, false, false, x, y, style, &family);
            ...
            BlockKind::TaskListItem {
                ...
                let indent = list_item_indent(marker, style);
                self.draw_list_marker(marker, true, *checked, x, y, style, &family);
                self.draw_blocks(children, x + indent, y);
            }
